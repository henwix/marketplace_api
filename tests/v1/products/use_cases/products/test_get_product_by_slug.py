...
def test_visible_product_retrieved_by_anonymous_user(
    ...
    retrieved_product = get_product_by_slug_use_case.execute(seller=None, slug=product.slug)
    db_product = (
...
def test_visible_product_retrieved_by_authorized_user_with_seller_profile(
    ...
    retrieved_product = get_product_by_slug_use_case.execute(seller=seller_to_entity(dto=seller), slug=product.slug)
    db_product = (
        Product.objects.filter(pk=product.pk)
        .prefetch_related(Prefetch('variants', ProductVariant.objects.filter(is_visible=True, price__gt=0)))
        .select_related('seller')
...
@pytest.mark.django_db
def test_not_visible_product_not_retrieved_by_anonymous_user(get_product_by_slug_use_case: GetProductBySlugUseCase):
    product = ProductModelFactory.create(is_visible=False)
    with pytest.raises(ProductAuthorPermissionError):
        get_product_by_slug_use_case.execute(seller=None, slug=product.slug)


@pytest.mark.django_db
def test_not_visible_product_not_retrieved(seller: Seller, get_product_by_slug_use_case: GetProductBySlugUseCase):
    product = ProductModelFactory.create(is_visible=False)
    with pytest.raises(ProductAuthorPermissionError):
        get_product_by_slug_use_case.execute(seller=seller_to_entity(dto=seller), slug=product.slug)


@pytest.mark.django_db
def test_not_visible_product_retrieved_by_author(seller: Seller, get_product_by_slug_use_case: GetProductBySlugUseCase):
...
@pytest.mark.django_db
def test_product_retrieved_with_relations(product: Product, get_product_by_slug_use_case: GetProductBySlugUseCase):
    expected_variants = 7
...
@pytest.mark.django_db
def test_product_not_retrieved_if_not_exists(get_product_by_slug_use_case: GetProductBySlugUseCase):
    with pytest.raises(ProductNotFoundBySlugError):
        get_product_by_slug_use_case.execute(seller=None, slug='test-slug')
