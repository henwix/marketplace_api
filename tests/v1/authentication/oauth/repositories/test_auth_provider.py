...
@pytest.fixture
def auth_provider_repository(container: Container) -> BaseAuthProviderRepository:
    return container.resolve(BaseAuthProviderRepository)
...
def test_get_provivder_by_uid_and_name_returns_provider_entity(
    ...
    auth_provider_entity = auth_provider_repository.get_by_provider_uid_and_name(
...
@pytest.mark.django_db
def test_get_provider_by_uid_and_name_returns_none_if_provider_does_not_exist(
    auth_provider_repository: BaseAuthProviderRepository,
):
    assert (
        auth_provider_repository.get_by_provider_uid_and_name(provider_uid=uuid4().hex, provider='test_provider')
        is None
    )
...
def test_auth_provider_successfully_saved(
    ...
    created_auth_provider = auth_provider_repository.save(auth_provider=new_auth_provider_entity, update=False)
...
def test_auth_provider_not_saved_with_sane_name_and_uid_and_auth_provider_provider_already_connected_error_raised(
    ...
    with pytest.raises(AuthProviderAlreadyConnectedError):
        auth_provider_repository.save(auth_provider=new_auth_provider_entity, update=False)
...
def test_get_many_by_user_id_returns_empty_list_if_no_connected_providers(
    ...
    result = auth_provider_repository.get_many_by_user_id(user_id=user.pk)
...
def test_get_many_by_user_id_returns_correct_data_(
    ...
    retrieved_auth_providers = auth_provider_repository.get_many_by_user_id(user_id=user.pk)
