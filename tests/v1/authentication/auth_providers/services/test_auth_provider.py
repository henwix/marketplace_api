...
def test_get_many_for_update_by_user_id_returns_empty_list_if_no_connected_providers(
    ...
    result = auth_provider_service.get_many_by_user_id_for_update(user_id=user.pk)
    assert isinstance(result, list)
    assert len(result) == 0
...
def test_get_many_for_update_by_user_id_returns_correct_data(
    ...
    retrieved_auth_providers = auth_provider_service.get_many_by_user_id_for_update(user_id=user.pk)
    assert isinstance(retrieved_auth_providers, list)
    assert len(retrieved_auth_providers) == expected_providers_number
...
def test_delete_by_user_id_and_provider_returns_true_if_deleted(
    ...
    is_deleted = auth_provider_service.try_delete_by_user_id_and_provider(
...
@pytest.mark.django_db
def test_delete_by_user_id_and_provider_raises_error_if_not_deleted(
    auth_provider_service: BaseAuthProviderService,
    user: User,
):
    assert AuthProvider.objects.filter(user_id=user.pk).count() == 0
    with pytest.raises(AuthProviderNotConnectedError):
        auth_provider_service.try_delete_by_user_id_and_provider(user_id=user.pk, provider='test_provider')
