...
def test_update_seller_updated(update_seller_use_case: UpdateSellerUseCase, seller: Seller):
    expected_data = {
        ...
        'description': 'test sellert desc',
    }
    updated_seller = update_seller_use_case.execute(user_id=seller.user_id, data=expected_data)
    ...
    assert db_seller.name == expected_data['name']
    assert db_seller.description == expected_data['description']
...
def test_update_seller_seller_not_found_error_raised(
    ...
    with pytest.raises(SellerNotFoundError):
        update_seller_use_case.execute(user_id=user.pk, data={})
