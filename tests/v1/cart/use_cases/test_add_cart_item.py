...
@pytest.fixture
def add_item_to_cart_use_case(container: Container) -> AddCartItemUseCase:
    return container.resolve(AddCartItemUseCase)


@pytest.mark.django_db
@pytest.mark.parametrize('expected_quantity', [1, 7, 13, 22, 33, 125, 542, 1002, 3521])
def test_add_item_with_existing_cart(
    add_item_to_cart_use_case: AddCartItemUseCase,
    user: User,
...
def test_add_item_not_added_and_cart_limit_error_raised(
    ...
    CartItemModelFactory.create_batch(size=50, cart=cart, seller=seller)
    with pytest.raises(CartLimitError):
        command = AddCartItemCommand(user_id=cart.user_id, product_variant_id=product_variant.id, quantity=1)
        add_item_to_cart_use_case.execute(command=command)


@pytest.mark.django_db
def test_add_item_credentials_error_raised(add_item_to_cart_use_case: AddCartItemUseCase):
    command = AddCartItemCommand(user_id=None, product_variant_id=uuid7(), quantity=1)
    with pytest.raises(AuthCredentialsNotProvidedError):
        add_item_to_cart_use_case.execute(command=command)


@pytest.mark.django_db
def test_add_item_user_not_found_error_raised(add_item_to_cart_use_case: AddCartItemUseCase):
    command = AddCartItemCommand(user_id=1, product_variant_id=uuid7(), quantity=1)
    with pytest.raises(UserNotFoundError):
        add_item_to_cart_use_case.execute(command=command)


@pytest.mark.django_db
def test_add_item_user_not_active_error_raised(add_item_to_cart_use_case: AddCartItemUseCase):
    user = UserModelFactory.create(is_active=False)
    command = AddCartItemCommand(user_id=user.pk, product_variant_id=uuid7(), quantity=1)
